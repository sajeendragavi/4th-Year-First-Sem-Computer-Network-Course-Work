...
def HELO(args, s, client_address, state):   #client greets the server and introduces itself
    ...
    if state['HELO'] == False:
      ...
      with open(fileName, 'a') as the_file:    #append the file name
        the_file.write(" ".join(args) + "\n")     #write in the file
      state['HELO'] = False
      state['MAIL'] = False
      state['RCPT'] = False
...
def MAIL(args, s, client_address, state):     # to establish the return address, also called return-path sender.
...
def RCPT(args, s, client_address, state):    # to establish a recipient of the message
...
def DATA(args, s, client_address, state):   #to signal the beginning of the message text; the content of the message
  fileName = str(state['file']) + '.txt'
  if state['MAIL'] == True and state['HELO'] == True and state['RCPT'] == True:
    s.send("354 End data with <CR><LF>.<CR><LF> \n".encode())
    data = recieveData(s, state)
    print('check data:%s',data)
    with open(fileName, 'a') as the_file:
      the_file.write("data \n")
      the_file.write(data)
   ...
   # with open(fileName) as f:                         #add to copy info to another text file
       # with open("mail_info.txt", "w") as f1:
           # for line in f:
             #   if "ROW" in line:
               #     f1.write(line)
#
